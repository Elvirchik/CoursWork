...
const Admin = () => {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`http://localhost:8080/users/getAll?page=0&size=1000`); // Получаем все пользователи для фильтрации
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
            // Фильтруем, чтобы не показывать текущего пользователя
            const filteredUsers = data.content.filter(user => user.id !== currentUserId);
    ...
    }, [allUsers, firstNameSearch, lastNameSearch, emailSearch, phoneSearch, roleFilter, currentPage, USERS_PER_PAGE]);
    ...
    const handleSave = async () => {
        try {
            const selectedRole = roles.find(role => role.id === parseInt(editedUser.role));
            ...
            const response = await fetch(`http://localhost:8080/users/update/${editUserId}`, {
                method: 'PUT',
                headers: {
    ...
    if (userRole !== 'ADMIN') {
        return <AuthRequired message="Доступ к админ-панели разрешен только администраторам." />;
    }
    ...
    return (
        <div id="admin">
            <h3 className='admin_button'> 
                ...
                <span className='Link'>
                    <Link to="/admin/all/orders" className='Link'>Заказы</Link> 
                </span>
                <span className='Link'>
            ...
            {showDeleteConfirm && (
                <div className="delete-confirmation-modal">
                    <div className="delete-confirmation-content">
                        ...
                        <p>Вы уверены, что хотите удалить пользователя {userToDelete.firstName} {userToDelete.lastName}?</p>
                        <div className="confirmation-buttons">
                            <button onClick={handleDelete} className="confirm-delete-btn">Удалить</button>
                            <button onClick={cancelDelete} className="cancel-delete-btn">Отмена</button>
                        </div>
                    </div>
                </div>
            )}
                                ...
                                {editUserId === user.id ? (
                                    ...
                                    <div className="txt_admin">
                                        ...
                                        <div className="buttons">
                                            ...
                                            <button onClick={() => confirmDelete(user)}>Удалить</button>
                                            <Link to={`/admin/user/${user.id}/orders`} className='show_order_admin'>Посмотреть заказы</Link>
                                        </div>
                                    </div>
                                )}
                            </div>
