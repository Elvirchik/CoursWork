...
const AllOrders = () => {
    ...
    const [statuses, setStatuses] = useState(["формируется", "на обработке", "собирается", "доставляется","доставлен", "завершен", "откланён"]);
    ...
    }, [statusFilter, currentPage, firstNameSearch, lastNameSearch, emailSearch, phoneSearch]);
    ...
    const handleDeleteOrder = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/orders/delete/${orderToDelete}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
                ...
                <div className="delete-confirmation">
                    <div className="delete-confirmation-content">
                        ...
                        <div className="delete-confirmation-buttons">
                            <button onClick={handleDeleteOrder} className="confirm-button">Да, удалить</button>
                            <button onClick={cancelDelete} className="cancel-button">Отмена</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    {(orders.length > 0) ? (
                        orders.map(order => (
                            <div className="one_zakaz" key={order.id}>
                                 <button className='delete delete_orders_admin' onClick={() => confirmDeleteOrder(order.id)}><span><FaTrash /></span> </button>
                                <div className="txt_admin">
                                    ...
                                    <div>
                                        ...
                                        <div className="not_upper">
                                            {order.customerLastName} {order.customerFirstName} ({order.customerEmail}, {order.customerPhone})
                                        </div>
                                    </div>
                                    <div>
